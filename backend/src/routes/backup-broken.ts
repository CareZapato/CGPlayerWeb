      ...
      try {
        ...
        if (dbContent.trim().startsWith('{')) {
          ...
          for (let i = 1; i < sections.length; i += 2) {
            ...
            try {
              // Buscar el JSON después de "-- Records: X" con regex más flexible
              console.log(`🔍 Buscando patrón en contenido de ${tableName}...`);
              
              // Intentar múltiples patrones de regex
              let recordsMatch = null;
              let jsonStr = '';
              
              // Patrón 1: -- Records: X seguido directamente de JSON
              const pattern1 = /-- Records: (\d+)\s*\n([\s\S]*?)(?=\n-- |$)/;
              recordsMatch = tableContent.match(pattern1);
              
              if (recordsMatch && recordsMatch[2]) {
                jsonStr = recordsMatch[2].trim();
                console.log(`📋 Patrón 1 exitoso: ${recordsMatch[1]} registros`);
              } else {
                // Patrón 2: buscar directamente el JSON que empiece con [ o {
                const pattern2 = /\[([\s\S]*?)\]/;
                const arrayMatch = tableContent.match(pattern2);
                if (arrayMatch && arrayMatch[0]) {
                  jsonStr = arrayMatch[0].trim();
                  console.log(`📋 Patrón 2 exitoso: encontrado array JSON`);
                } else {
                  // Patrón 3: buscar después de cualquier "Records: X"
                  const pattern3 = /Records: \d+[^\[]*(\[[\s\S]*?\])/;
                  const match3 = tableContent.match(pattern3);
                  if (match3 && match3[1]) {
                    jsonStr = match3[1].trim();
                    console.log(`📋 Patrón 3 exitoso: encontrado JSON después de Records`);
                  }
                }
              }
