...
router.post('/backup/restore', authenticateToken, requireAdmin, upload.single('backup'), async (req: Request, res: Response) => {
  try {
    ...
    if (!fs.existsSync(backupInfoPath)) {
      throw new Error('Archivo de backup inválido: falta backup-info.json');
    }
    ...
    try {
      ...
      console.log('📖 Archivo de base de datos leído correctamente');
      console.log(`📏 Tamaño del archivo: ${dbContent.length} caracteres`);
      console.log(`🔍 Primeros 100 caracteres: "${dbContent.substring(0, 100)}..."`);
      ...
      try {
        ...
        if (dbContent.trim().startsWith('{')) {
          ...
          // Si no es JSON puro, buscar si hay JSON embebido (formato SQL con JSON)
          console.log('⚠️ No es formato JSON puro, buscando JSON embebido...');
          const jsonMatch = dbContent.match(/\{[\s\S]*\}/);
          if (jsonMatch) {
            data = JSON.parse(jsonMatch[0]);
            console.log('📋 JSON embebido encontrado y parseado correctamente');
          } else {
            throw new Error('No se pudo encontrar datos JSON válidos en el backup');
          }
        }
      } catch (parseError) {
      ...
      console.log('🔍 Estructura del backup encontrada:');
