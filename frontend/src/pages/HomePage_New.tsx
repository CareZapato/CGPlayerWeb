    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl text-white p-4 sm:p-6 mb-6 sm:mb-8">
        <div className="text-center">
          ...
          <p className="text-base sm:text-lg opacity-90">Bienvenido a CGPlayer v0.9.0</p>
          
          {/* Mostrar tipos de voz del usuario si los tiene */}
          {user.voiceProfiles && user.voiceProfiles.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {user.voiceProfiles.map((profile) => (
                <span
                  key={profile.id}
                  className="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-white/20 text-white border border-white/30 backdrop-blur-sm"
                >
